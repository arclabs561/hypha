use hypha::{SporeNode, PhysicalState, Capability, Task};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for dead_percentage in [0, 20, 40, 60, 80] {
        println!("Evaluating Mycelium with {}% low-power nodes...", dead_percentage);
    ...
    for res in results {
        println!("{}% Dead -> {} Bids", res["dead_percentage"], res["successful_bids"]);
    }
