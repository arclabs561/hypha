...
impl MeshPeer {
    ...
    pub fn score(&self) -> f32 {
        ...
        self.energy_score * 0.3 + activity_score * 0.2 + normalized_conductivity * 0.3 + pressure_score * 0.2
    }
}
...
impl TopicMesh {
    ...
    pub fn align_pulse(&mut self, neighbor_phase: f32, weight: f32) {
        ...
        // Handle wrap-around
        let diff = if diff > 0.5 { diff - 1.0 } else if diff < -0.5 { diff + 1.0 } else { diff };
        self.pulse_phase = (self.pulse_phase + diff * weight) % 1.0;
        if self.pulse_phase < 0.0 { self.pulse_phase += 1.0; }
    }
    ...
    pub fn add_peer(&mut self, id: String, energy_score: f32) {
        self.known_peers.entry(id.clone())
    ...
    pub fn mesh_median_score(&self) -> f32 {
        let mut scores: Vec<f32> = self.mesh_peers.iter()
    ...
    pub fn heartbeat(&mut self) -> Vec<(String, MeshControl)> {
        ...
        let to_prune: Vec<String> = self.mesh_peers.iter()
            .filter(|id| {
        ...
        for id in to_prune {
            ...
            controls.push((id.clone(), MeshControl::Prune {
        ...
        while self.mesh_peers.len() > self.config.d_high {
            ...
            let lowest = self.mesh_peers.iter()
        ...
        while self.mesh_peers.len() < self.config.d_low {
            ...
            let candidate = self.known_peers.iter()
                .filter(|(id, peer)| {
            ...
            if let Some((id, _)) = candidate {
                ...
                controls.push((id, MeshControl::Graft {
                    topic: self.topic.clone(),
                }));
            } else {
        ...
        if median < self.config.opportunistic_graft_threshold && self.mesh_peers.len() < self.config.d_high {
            let candidates: Vec<_> = self.known_peers.iter()
                .filter(|(id, peer)| {
                    !self.mesh_peers.contains(*id) &&
                    !self.backoff.contains_key(*id) &&
                    peer.score() > median
        ...
        if self.mesh_peers.len() >= self.config.d_low {
            let weakest = self.mesh_peers.iter()
                .filter_map(|id| self.known_peers.get(id).map(|p| (id.clone(), p.score())))
                .min_by(|a, b| a.1.partial_cmp(&b.1).unwrap());
            
            if let Some((weak_id, weak_score)) = weakest {
                let best_candidate = self.known_peers.iter()
                    .filter(|(id, peer)| {
                        !self.mesh_peers.contains(*id) &&
                        !self.backoff.contains_key(*id) &&
                ...
                if let Some((best_id, _)) = best_candidate {
                    ...
                    controls.push((weak_id.clone(), MeshControl::Prune {
                        topic: self.topic.clone(),
                    ...
                    controls.push((best_id, MeshControl::Graft {
                        topic: self.topic.clone(),
                    }));
                }
            }
        ...
        let non_mesh: Vec<_> = self.known_peers.keys()
            .filter(|id| !self.mesh_peers.contains(*id))
            .cloned()
            .collect();
        
        let ihave_targets: Vec<_> = non_mesh.choose_multiple(&mut rng, self.config.d_lazy.min(non_mesh.len()))
            .cloned()
            .collect();
        
        if !self.message_cache.is_empty() && !ihave_targets.is_empty() {
            let recent_msgs: Vec<_> = self.message_cache.iter()
                .take(10)
                .cloned()
                .collect();
            
            for target in ihave_targets {
                controls.push((target, MeshControl::IHave {
                    topic: self.topic.clone(),
    ...
    pub fn handle_graft(&mut self, peer_id: &str) -> bool {
        if let Some(peer) = self.known_peers.get(peer_id) {
            if peer.score() >= self.config.graft_threshold && self.mesh_peers.len() < self.config.d_high {
    ...
    pub fn handle_prune(&mut self, peer_id: &str, backoff: Duration) {
        ...
        self.backoff.insert(peer_id.to_string(), Instant::now() + backoff);
    }
    ...
    pub fn handle_control(&mut self, peer_id: &str, control: MeshControl) -> Option<MeshControl> {
        match control {
            ...
            MeshControl::IHave { message_ids, .. } => {
                let missing: Vec<_> = message_ids.into_iter()
                    .filter(|id| !self.message_cache.contains(id))
                    .collect();
                
                if !missing.is_empty() {
                    Some(MeshControl::IWant { message_ids: missing })
                } else {
                    None
                }
            }
    ...
    pub fn stats(&self) -> MeshStats {
        let scores: Vec<f32> = self.mesh_peers.iter()
            .filter_map(|id| self.known_peers.get(id))
            .map(|p| p.score())
            .collect();
...
mod tests {
    ...
    fn test_spike_handling() {
        ...
        let initial_pressure = mesh.local_pressure;
        
        // Handle danger spike
        mesh.handle_spike("danger-node", 255);
        
        assert_eq!(mesh.local_pressure, 10.0, "Pressure should max out on danger spike");
        let final_cond = mesh.known_peers.get("danger-node").unwrap().conductivity;
        assert!(final_cond >= initial_cond + 2.0, "Path should thicken significantly");
    }
}
